...
function initNVDACountdown() {
  const countdownEl = document.getElementById('nvda-countdown');
  if (!countdownEl) return;
  
  // NVDA earnings: February 26, 2026 after market close (4:00 PM EST)
  ...
  function updateCountdown() {
    ...
    if (days > 0) {
      ...
      countdownEl.classList.add('text-accent-red');
    }
  }
...
function copyNVDANotes() {
  const notes = `NVIDIA (NVDA) Q4 FY2026 Earnings — Feb 26, 2026

📊 KEY METRICS:
• EPS Consensus: $1.23–1.26 (up 60% YoY)
• Forward P/E: ~40.7x (based on $4.69 FY EPS)
• OpEx Growth: High-30% range (FY2026 guidance)
• Other Income: ~$500M (GAAP/non-GAAP)

📈 ANALYST SENTIMENT: Bullish
• Motley Fool prediction: Stock poised to soar post-earnings
• Beat scenario: $1.30+ EPS would exceed consensus
• Key focus: Data center revenue growth, AI demand sustainability

🔗 SOURCES:
• IG Earnings Preview: https://www.ig.com/en/news-and-trade-ideas/nvidia-q4-2026-earnings-preview-260217
• Motley Fool Analysis: https://www.fool.com/investing/2026/02/03/prediction-nvidia-stock-going-to-soar-after-feb-25/

Generated by Nox Dashboard — ${new Date().toLocaleDateString()}
`;
  
  navigator.clipboard.writeText(notes).then(() => {
    alert('NVDA earnings notes copied to clipboard!');
  }).catch(() => {
    alert('Failed to copy notes. Try selecting and copying manually.');
  });
}

// ==================== MINECRAFT 26.1 CONTENT PLANNER ====================

const mc26BabyMobs = [
  { name: 'Baby Axolotl', type: 'Aquatic', cuteness: 'High', contentAngle: 'Aquarium builds, bucket tricks' },
  { name: 'Baby Dolphin', type: 'Aquatic', cuteness: 'High', contentAngle: 'Ocean exploration, jump challenges' },
  { name: 'Baby Turtle', type: 'Aquatic', cuteness: 'Medium', contentAngle: 'Beach houses, egg protection' },
  { name: 'Baby Squid', type: 'Aquatic', cuteness: 'Medium', contentAngle: 'Underwater bases, ink farms' },
  { name: 'Baby Glow Squid', type: 'Aquatic', cuteness: 'High', contentAngle: 'Cave lighting, aesthetic builds' },
  { name: 'Baby Panda', type: 'Land', cuteness: 'Very High', contentAngle: 'Bamboo farms, lazy animations' },
  { name: 'Baby Fox', type: 'Land', cuteness: 'High', contentAngle: 'Arctic builds, item stealing' },
  { name: 'Baby Bee', type: 'Flying', cuteness: 'High', contentAngle: 'Auto farms, pollination' },
  { name: 'Baby Cat', type: 'Land', cuteness: 'Very High', contentAngle: 'Cozy houses, morning gifts' },
  { name: 'Baby Wolf', type: 'Land', cuteness: 'High', contentAngle: 'Survival series, combat help' },
  { name: 'Baby Horse', type: 'Land', cuteness: 'Medium', contentAngle: 'Racing, breeding programs' },
  { name: 'Baby Donkey', type: 'Land', cuteness: 'Medium', contentAngle: 'Storage systems, caravan builds' },
  { name: 'Baby Llama', type: 'Land', cuteness: 'Medium', contentAngle: 'Caravans, spit defense' },
  { name: 'Baby Hoglin', type: 'Nether', cuteness: 'Low', contentAngle: 'Nether farms, fear mechanics' },
  { name: 'Baby Strider', type: 'Nether', cuteness: 'High', contentAngle: 'Lava boats, nether transport' }
];

function initMC26Planner() {
  const container = document.getElementById('mc26-content-planner');
  if (!container) return;
    ...
    <div class="card rounded-lg p-4 border-l-4 border-accent-green">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-lg">Minecraft 26.1 Content Planner</h3>
            <p class="text-sm text-gray-400">First Drop 2026 — All 15 Baby Mobs + New Features</p>
          </div>
        </div>
        <div class="mt-2 md:mt-0 text-right">
          <div class="text-xs text-gray-400">Expected Release</div>
          <div class="text-xl font-bold text-accent-green">Late March 2026</div>
        </div>
      </div>

      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-blue">15</div>
          <div class="text-xs text-gray-400">Baby Mobs</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-yellow">∞</div>
          <div class="text-xs text-gray-400">Cuteness</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-purple">✓</div>
          <div class="text-xs text-gray-400">Craftable Nametags</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-green">🌼</div>
          <div class="text-xs text-gray-400">Golden Dandelion</div>
        </div>
      </div>

      <div class="bg-dark-800 rounded-lg p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-medium">📸 Content Ideas by Mob Type</span>
          <button onclick="copyMC26Ideas()" class="text-xs px-2 py-1 bg-accent-blue rounded hover:bg-blue-600 transition-colors">📋 Copy All</button>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
          ${mc26BabyMobs.map(mob => `
            <div class="bg-dark-700 rounded p-2 text-sm">
              <div class="flex items-center justify-between">
                <span class="font-semibold">${mob.name}</span>
                <span class="text-xs px-2 py-0.5 bg-dark-600 rounded">${mob.type}</span>
              </div>
              <div class="text-xs text-gray-400 mt-1">${mob.contentAngle}</div>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3">
          <div class="text-sm font-semibold mb-2">🎬 Video Concepts</div>
          <ul class="text-xs text-gray-300 space-y-1">
            <li>• "All 15 Baby Mobs Ranked by Cuteness"</li>
            <li>• "I Built a Zoo for Every Baby Mob"</li>
            <li>• "Baby Mob Battle Royale"</li>
            <li>• "Finding the Rarest Baby Mob"</li>
          </ul>
        </div>
        <div class="bg-dark-800 rounded-lg p-3">
          <div class="text-sm font-semibold mb-2">🔧 Feature Videos</div>
          <ul class="text-xs text-gray-300 space-y-1">
            <li>• "Craftable Nametags Finally!"</li>
            <li>• "Golden Dandelion Farm"</li>
            <li>• "26.1 Complete Feature Guide"</li>
            <li>• "Before vs After: All Baby Mobs"</li>
          </ul>
        </div>
        <div class="bg-dark-800 rounded-lg p-3">
          <div class="text-sm font-semibold mb-2">📅 Timeline</div>
          <ul class="text-xs text-gray-300 space-y-1">
            <li>• Now: Prep world saves</li>
            <li>• March: Snapshot content</li>
            <li>• Release Day: First impressions</li>
            <li>• Week 1: Deep dive series</li>
          </ul>
        </div>
      </div>

      <div class="flex gap-2">
        <a href="https://minecraft.wiki/w/Java_Edition_26.1" target="_blank" rel="noopener" class="px-3 py-2 bg-accent-blue hover:bg-blue-600 rounded text-sm transition-colors">📖 Minecraft Wiki</a>
        <a href="https://www.minecraft.net/en-us/article/minecraft-26-1-snapshot-8" target="_blank" rel="noopener" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">🔬 Snapshot 8</a>
        <button onclick="generateMC26Script()" class="px-3 py-2 bg-accent-green hover:bg-green-600 rounded text-sm transition-colors">🎬 Generate Script</button>
      </div>
    </div>
  `;
}

function copyMC26Ideas() {
  const ideas = mc26BabyMobs.map(mob => `${mob.name} (${mob.type}): ${mob.contentAngle}`).join('\n');
  const fullText = `Minecraft 26.1 Content Ideas — First Drop 2026

All 15 Baby Mobs with Content Angles:
${ideas}

Video Concepts:
• "All 15 Baby Mobs Ranked by Cuteness"
• "I Built a Zoo for Every Baby Mob"
• "Baby Mob Battle Royale"
• "Finding the Rarest Baby Mob"
• "Craftable Nametags Finally!"
• "26.1 Complete Feature Guide"
...
`;
  ...
  navigator.clipboard.writeText(fullText).then(() => {
    alert('Minecraft 26.1 content ideas copied!');
  });
}

function generateMC26Script() {
  const script = `MINECRAFT 26.1 — ALL 15 BABY MOBS (First Drop 2026)

[COLD OPEN — 0:00-0:15]
Show quick cuts of the cutest new baby mobs. End on baby panda sneezing.
"Minecraft 26.1 just dropped, and every baby mob got a complete makeover."

[INTRO — 0:15-0:45]
"This is First Drop 2026 — the spring update that adds new textures and models for every baby mob that didn't already have a unique model. That's 15 total baby mobs, all cuter than ever. Plus craftable nametags and a new golden dandelion. Let's check them all out."

[SEGMENT 1: AQUATIC BABIES — 0:45-2:30]
"First, the aquatic update. Baby axolotls, dolphins, turtles, squid, and glow squid all have new models."
[Show each with before/after if possible]
"The baby glow squid is honestly adorable — those tiny tentacles."

[SEGMENT 2: LAND BABIES — 2:30-5:00]
"Now the land mobs. Baby pandas, foxes, cats, wolves, horses, donkeys, llamas — all redesigned."
[Show panda playing, fox sleeping, cat giving morning gifts]
"The baby panda animations are next level."

[SEGMENT 3: NETHER BABIES — 5:00-6:00]
"Even the Nether got love. Baby hoglins and striders."
[Show strider on lava]
"Look at this little guy. He's so happy on his lava."

[SEGMENT 4: NEW FEATURES — 6:00-7:30]
"Two huge gameplay changes. First: craftable nametags!"
[Show crafting recipe]
"No more dungeon hunting. Just craft them."
"And the golden dandelion — new flower type."

[OUTRO — 7:30-8:00]
"That's Minecraft 26.1 — First Drop 2026. 15 adorable baby mobs, craftable nametags, and more. Which baby mob is your favorite? Comment below. Subscribe for more Minecraft content."
...
// Auto-init when YouTube tab outliers section is shown
document.getElementById('tab-btn-youtube')?.addEventListener('click', () => {
  setTimeout(initMC26Planner, 200);
  setTimeout(initShortsLongMixer, 300);
});

// Auto-init countdown when investments tab is shown
document.getElementById('tab-btn-investments')?.addEventListener('click', () => {
  setTimeout(initNVDACountdown, 100);
});

// ==================== SHORTS-TO-LONG FORM CONTENT MIXER ====================

const shortsLongTemplates = [
  {
    name: 'The Teaser Trailer',
    shortsHook: 'Show the most shocking 5 seconds, cut to black',
    longFormPayoff: 'Full context, buildup, and aftermath explained',
    bestFor: 'Mystery reveals, rare finds, unexpected moments',
    example: 'Short: "You won\'t believe what spawned..." | Long: Full mob farm tour'
  },
  {
    name: 'Transformation Reveal',
    shortsHook: 'Before state → quick transition → after state',
    longFormPayoff: 'Step-by-step process, mistakes, lessons learned',
    bestFor: 'Builds, makeovers, progress videos',
    example: 'Short: 3-second build timelapse | Long: Full tutorial with commentary'
  },
  {
    name: 'Reaction Bait',
    shortsHook: 'Your genuine reaction to something shocking',
    longFormPayoff: 'Full analysis, research, context, related examples',
    bestFor: 'News, updates, viral moments, game changers',
    example: 'Short: "WAIT WHAT?!" face | Long: Full breakdown of 26.1 changes'
  },
  {
    name: 'Compilation Preview',
    shortsHook: '3 best moments from a longer session',
    longFormPayoff: 'Full uncut session with commentary',
    bestFor: 'Gameplay, funny moments, highlights',
    example: 'Short: 3 fails in 15 seconds | Long: Full 20-min gameplay'
  },
  {
    name: 'Tutorial Preview',
    shortsHook: 'End result with "Learn this in 60 seconds"',
    longFormPayoff: 'Detailed tutorial with alternatives and tips',
    bestFor: 'How-to content, tips, tricks, guides',
    example: 'Short: Fast redstone door | Long: 5 door designs explained'
  },
  {
    name: 'Challenge Setup',
    shortsHook: 'The rules + your confidence level',
    longFormPayoff: 'Full challenge attempt with all attempts',
    bestFor: 'Challenges, survival series, hardcore',
    example: 'Short: "Can I beat Minecraft blindfolded?" | Long: Full 3-hour attempt'
  }
];

function initShortsLongMixer() {
  const container = document.getElementById('shorts-long-mixer');
  if (!container) return;
    ...
    <div class="card rounded-lg p-4 border-l-4 border-accent-purple">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          <div class="text-3xl">🎬</div>
          <div>
            <h3 class="font-semibold text-lg">Shorts → Long-Form Content Mixer</h3>
            <p class="text-sm text-gray-400">2026 Trend: Shorts as trailers for long-form content</p>
          </div>
        </div>
        <button onclick="generateContentMix()" class="mt-2 md:mt-0 px-3 py-2 bg-accent-purple hover:bg-purple-600 rounded text-sm transition-colors">🎲 Generate Mix</button>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        ${shortsLongTemplates.map((t, i) => `
          <div class="bg-dark-800 rounded-lg p-3 cursor-pointer hover:border-accent-purple border border-transparent transition-colors" onclick="selectMixTemplate(${i})">
            <div class="flex items-center justify-between mb-2">
              <span class="font-semibold text-sm">${t.name}</span>
              <span class="text-xs px-2 py-0.5 bg-dark-600 rounded">${t.bestFor}</span>
            </div>
            <div class="text-xs text-gray-400 mb-1">🎣 ${t.shortsHook}</div>
            <div class="text-xs text-accent-green">🎬 ${t.longFormPayoff.substring(0, 50)}...</div>
          </div>
        `).join('')}
      </div>

      <div id="mix-output" class="hidden bg-dark-800 rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <h4 class="font-semibold">🎯 Your Content Mix</h4>
          <button onclick="copyMixPlan()" class="text-xs px-2 py-1 bg-accent-blue rounded hover:bg-blue-600">📋 Copy</button>
        </div>
        <div id="mix-content"></div>
      </div>

      <div class="mt-4 bg-dark-800 rounded-lg p-3">
        <div class="text-sm font-semibold mb-2">💡 2026 Content Strategy Tips</div>
        <ul class="text-xs text-gray-300 space-y-1">
          <li>• Post Shorts 2-3 hours BEFORE long-form for algorithm boost</li>
          <li>• Use same thumbnail style for visual consistency</li>
          <li>• Link to long-form in Shorts comments AND description</li>
          <li>• End Shorts with "Full video on my channel" CTA</li>
          <li>• Transformation videos consistently highest-viewed Shorts format</li>
        </ul>
      </div>
    </div>
  `;
...
let selectedMixTemplate = null;

function selectMixTemplate(index) {
  selectedMixTemplate = shortsLongTemplates[index];
  generateContentMix();
}

function generateContentMix() {
  if (!selectedMixTemplate) {
    selectedMixTemplate = shortsLongTemplates[Math.floor(Math.random() * shortsLongTemplates.length)];
  }
  ...
  content.innerHTML = `
    <div class="mb-3">
      <div class="text-xs text-gray-400 mb-1">Selected Format</div>
      <div class="font-semibold text-accent-purple">${selectedMixTemplate.name}</div>
    </div>
    
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div class="bg-dark-700 rounded p-3">
        <div class="text-xs text-accent-yellow mb-1">📱 SHORTS (15-60 sec)</div>
        <div class="text-sm mb-2">${selectedMixTemplate.shortsHook}</div>
        <div class="text-xs text-gray-500">Hook: First 3 seconds critical</div>
      </div>
      
      <div class="bg-dark-700 rounded p-3">
        <div class="text-xs text-accent-green mb-1">🎥 LONG-FORM (8-20 min)</div>
        <div class="text-sm mb-2">${selectedMixTemplate.longFormPayoff}</div>
        <div class="text-xs text-gray-500">Payoff: Deliver on Shorts promise</div>
      </div>
    </div>
    
    <div class="mt-3 bg-dark-700 rounded p-3">
      <div class="text-xs text-gray-400 mb-1">Best For</div>
      <div class="text-sm">${selectedMixTemplate.bestFor}</div>
      <div class="mt-2 text-xs text-gray-500">Example: ${selectedMixTemplate.example}</div>
    </div>
  `;
...
function copyMixPlan() {
  if (!selectedMixTemplate) return;
  
  const plan = `CONTENT MIX PLAN — ${selectedMixTemplate.name}

📱 SHORTS STRATEGY (15-60 seconds):
${selectedMixTemplate.shortsHook}
→ Hook viewers in first 3 seconds
→ End with CTA: "Full video on my channel"
→ Post 2-3 hours BEFORE long-form

🎥 LONG-FORM STRATEGY (8-20 minutes):
${selectedMixTemplate.longFormPayoff}
→ Deliver on the Shorts promise
→ Include deeper context and value
→ Link in Shorts comments + description

BEST FOR: ${selectedMixTemplate.bestFor}

EXAMPLE: ${selectedMixTemplate.example}

---
Generated by Nox Dashboard Shorts-Long Mixer
`;
  
  navigator.clipboard.writeText(plan).then(() => {
    alert('Content mix plan copied to clipboard!');
  });
}

// ==================== BBS CINEMATIC CONTENT PLANNER ====================

const bbsCinematicScenes = [
  { name: 'The Gathering Storm', type: 'Opening', mood: 'Dramatic', setup: 'Villagers preparing for danger', camera: 'Slow pan across village' },
  { name: 'Hero\'s Arrival', type: 'Character Intro', mood: 'Epic', setup: 'Main character enters with custom model', camera: 'Low angle reveal' },
  { name: 'The Confrontation', type: 'Action', mood: 'Tense', setup: 'Two factions face off', camera: 'Overhead wide shot' },
  { name: 'Emotional Beat', type: 'Drama', mood: 'Somber', setup: 'Character loss or sacrifice', camera: 'Close-up with shallow depth' },
  { name: 'The Battle', type: 'Action', mood: 'Intense', setup: 'Crowd spawner NPCs fighting', camera: 'Dynamic tracking shots' },
  { name: 'Victory Moment', type: 'Climax', mood: 'Triumphant', setup: 'Hero stands victorious', camera: 'Slow motion rotation' },
  { name: 'Peace Restored', type: 'Ending', mood: 'Hopeful', setup: 'Village rebuilding', camera: 'Sunrise time-lapse' },
  { name: 'The Journey Begins', type: 'Opening', mood: 'Adventure', setup: 'Character leaves home', camera: 'Following shot into wilderness' }
];

const bbsCameraTechniques = [
  { name: 'Dolly Zoom', effect: 'Vertigo/disorientation', useCase: 'Character realization moments' },
  { name: 'Rack Focus', effect: 'Shift attention', useCase: 'Reveal something in background' },
  { name: 'Tracking Shot', effect: 'Follow action', useCase: 'Chase scenes, walking dialogue' },
  { name: 'Static Wide', effect: 'Establish scale', useCase: 'Opening shots, landscape reveals' },
  { name: 'Dutch Angle', effect: 'Unease/tension', useCase: 'Villain scenes, danger moments' },
  { name: 'Overhead', effect: 'God\'s eye view', useCase: 'Battle scenes, maze reveals' },
  { name: 'POV Shot', effect: 'Immersion', useCase: 'Character perspective moments' },
  { name: 'Slow Motion', effect: 'Drama/emphasis', useCase: 'Impact moments, emotional beats' }
];

function initBBSCinematicPlanner() {
  const container = document.getElementById('bbs-cinematic-planner');
  if (!container) return;
  
  container.innerHTML = `
    <div class="card rounded-lg p-4 border-l-4 border-accent-red">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          <div class="text-3xl">🎬</div>
          <div>
            <h3 class="font-semibold text-lg">BBS Cinematic Content Planner</h3>
            <p class="text-sm text-gray-400">Blockbuster Studio Mod — Machinima & Storytelling</p>
          </div>
        </div>
        <div class="flex gap-2 mt-2 md:mt-0">
          <button onclick="generateSceneSequence()" class="px-3 py-2 bg-accent-red hover:bg-red-600 rounded text-sm transition-colors">🎲 Generate Scene</button>
          <button onclick="copyCinematicPlan()" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">📋 Copy Plan</button>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-blue">BBS Reforge</div>
          <div class="text-xs text-gray-400">1.21.1 Available</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-yellow">8 Scenes</div>
          <div class="text-xs text-gray-400">Story Templates</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-2xl font-bold text-accent-green">8 Camera</div>
          <div class="text-xs text-gray-400">Techniques</div>
        </div>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3">
          <div class="text-sm font-semibold mb-2">🎭 Cinematic Scenes</div>
          <div class="space-y-1 max-h-40 overflow-y-auto">
            ${bbsCinematicScenes.map((scene, i) => `
              <div class="bg-dark-700 rounded p-2 text-xs cursor-pointer hover:bg-dark-600" onclick="selectScene(${i})">
                <div class="flex justify-between">
                  <span class="font-semibold">${scene.name}</span>
                  <span class="text-gray-400">${scene.type}</span>
                </div>
                <div class="text-gray-500 mt-0.5">${scene.mood} • ${scene.setup.substring(0, 30)}...</div>
              </div>
            `).join('')}
          </div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3">
          <div class="text-sm font-semibold mb-2">📹 Camera Techniques</div>
          <div class="space-y-1 max-h-40 overflow-y-auto">
            ${bbsCameraTechniques.map((tech, i) => `
              <div class="bg-dark-700 rounded p-2 text-xs cursor-pointer hover:bg-dark-600" onclick="selectCamera(${i})">
                <div class="flex justify-between">
                  <span class="font-semibold">${tech.name}</span>
                  <span class="text-accent-yellow">${tech.effect}</span>
                </div>
                <div class="text-gray-500 mt-0.5">${tech.useCase}</div>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      <div id="cinematic-output" class="hidden bg-dark-800 rounded-lg p-4 mb-4">
        <h4 class="font-semibold mb-3">🎬 Your Cinematic Scene</h4>
        <div id="cinematic-scene-content"></div>
      </div>

      <div class="bg-dark-800 rounded-lg p-3">
        <div class="text-sm font-semibold mb-2">💡 BBS Content Ideas</div>
        <ul class="text-xs text-gray-300 space-y-1">
          <li>• "I Made a Minecraft Movie Using BBS Mod" — Full cinematic story</li>
          <li>• "BBS vs Flashback: Which is Better for Cinematics?" — Comparison</li>
          <li>• "Recreating Famous Movie Scenes in Minecraft" — BBS showcase</li>
          <li>• "Crowd Spawner + BBS = Epic Battles" — Your mod combo</li>
          <li>• "BBS Tutorial: From First Scene to Full Movie" — Educational series</li>
        </ul>
      </div>

      <div class="mt-4 flex gap-2">
        <a href="https://www.curseforge.com/minecraft/mc-mods/bbsreforge" target="_blank" rel="noopener" class="px-3 py-2 bg-accent-blue hover:bg-blue-600 rounded text-sm transition-colors">⬇️ BBS Reforge</a>
        <a href="https://modrinth.com/modpack/bbs-movie-studio" target="_blank" rel="noopener" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">🎬 Movie Studio Pack</a>
        <a href="https://www.reddit.com/r/feedthebeast/comments/1f2duvg/ive_been_working_on_a_mod_bbs_mod_to_create/" target="_blank" rel="noopener" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">📖 Reddit Thread</a>
      </div>
    </div>
  `;
}
...
function selectScene(index) {
  selectedScene = bbsCinematicScenes[index];
  updateCinematicOutput();
}

function selectCamera(index) {
  selectedCamera = bbsCameraTechniques[index];
  updateCinematicOutput();
}

function generateSceneSequence() {
  selectedScene = bbsCinematicScenes[Math.floor(Math.random() * bbsCinematicScenes.length)];
  selectedCamera = bbsCameraTechniques[Math.floor(Math.random() * bbsCameraTechniques.length)];
  updateCinematicOutput();
}

function updateCinematicOutput() {
  const output = document.getElementById('cinematic-output');
  const content = document.getElementById('cinematic-scene-content');
  
  if (!selectedScene && !selectedCamera) return;
  
  let html = '';
  
  if (selectedScene) {
    html += `
      <div class="mb-3 bg-dark-700 rounded p-3">
        <div class="text-xs text-gray-400 mb-1">Selected Scene</div>
        <div class="font-semibold text-accent-blue">${selectedScene.name}</div>
        <div class="text-xs text-gray-300 mt-1">${selectedScene.setup}</div>
        <div class="text-xs text-gray-500 mt-1">Mood: ${selectedScene.mood} • Camera: ${selectedScene.camera}</div>
      </div>
    `;
  }
  ...
  if (selectedCamera) {
    html += `
      <div class="mb-3 bg-dark-700 rounded p-3">
        <div class="text-xs text-gray-400 mb-1">Camera Technique</div>
        <div class="font-semibold text-accent-yellow">${selectedCamera.name}</div>
        <div class="text-xs text-gray-300 mt-1">Effect: ${selectedCamera.effect}</div>
        <div class="text-xs text-gray-500 mt-1">Use for: ${selectedCamera.useCase}</div>
      </div>
    `;
  }
...
function copyCinematicPlan() {
  if (!selectedScene && !selectedCamera) {
    alert('Generate or select a scene first!');
    return;
  }
  
  let plan = `BBS CINEMATIC PLAN

`;
  
  if (selectedScene) {
    plan += `SCENE: ${selectedScene.name}
Type: ${selectedScene.type}
Mood: ${selectedScene.mood}
Setup: ${selectedScene.setup}
Camera Direction: ${selectedScene.camera}

`;
  }
  
  if (selectedCamera) {
    plan += `CAMERA: ${selectedCamera.name}
Effect: ${selectedCamera.effect}
Use Case: ${selectedCamera.useCase}

`;
  }
  
  plan += `BBS TIPS:
• Use CustomNPCs + BBS for character models
• Record with Replay Mod or Flashback for smooth shots
• Use Crowd Spawner for large battle scenes
• Plan shots before recording — BBS is for scripted content

Resources:
• BBS Reforge: https://www.curseforge.com/minecraft/mc-mods/bbsreforge
• Movie Studio Pack: https://modrinth.com/modpack/bbs-movie-studio

Generated by Nox Dashboard — ${new Date().toLocaleDateString()}
`;
  
  navigator.clipboard.writeText(plan).then(() => {
    alert('Cinematic plan copied to clipboard!');
  });
}

// Auto-init when YouTube tab is clicked
document.getElementById('tab-btn-youtube')?.addEventListener('click', () => {
  setTimeout(initBBSCinematicPlanner, 400);
  setTimeout(initMarketplaceBenchmark, 500);
});

// ==================== MARKETPLACE CREATOR BENCHMARK ====================

const marketplaceStats = {
  totalRevenue: 560000000, // $560M in past 12 months
  q1Record: 146000000, // Q1 2025 record
  creatorPayouts: 500000000, // $500M+ to creators
  mobileQuarterly: 42300000, // $42.3M mobile earnings quarterly
  avgCreatorEarnings: 50000, // estimated average
  topCreatorEarnings: 1000000 // $1M+ for top creators
};

const creatorTiers = [
  { name: 'New Creator', minSales: 0, maxSales: 100, label: 'Just Starting', color: 'gray' },
  { name: 'Growing Creator', minSales: 100, maxSales: 1000, label: 'Building Audience', color: 'blue' },
  { name: 'Established Creator', minSales: 1000, maxSales: 10000, label: 'Regular Income', color: 'green' },
  { name: 'Pro Creator', minSales: 10000, maxSales: 50000, label: 'Full-time Potential', color: 'yellow' },
  { name: 'Top Creator', minSales: 50000, maxSales: 1000000, label: 'Elite Tier', color: 'purple' }
];

function initMarketplaceBenchmark() {
  const container = document.getElementById('marketplace-benchmark');
  if (!container) return;
    ...
    <div class="card rounded-lg p-4 border-l-4 border-accent-green">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          <div class="text-3xl">💎</div>
          <div>
            <h3 class="font-semibold text-lg">Marketplace Creator Benchmark</h3>
            <p class="text-sm text-gray-400">See how you compare to marketplace averages</p>
          </div>
        </div>
        <div class="text-right mt-2 md:mt-0">
          <div class="text-xs text-gray-400">Total Creator Payouts</div>
          <div class="text-xl font-bold text-accent-green">$500M+</div>
        </div>
      </div>

      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-blue">$560M</div>
          <div class="text-xs text-gray-400">12-Month Revenue</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-yellow">$146M</div>
          <div class="text-xs text-gray-400">Q1 2025 Record</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-purple">$42.3M</div>
          <div class="text-xs text-gray-400">Mobile/Quarter</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-green">$1M+</div>
          <div class="text-xs text-gray-400">Top Creator Tier</div>
        </div>
      </div>

      <div class="bg-dark-800 rounded-lg p-4 mb-4">
        <div class="text-sm font-semibold mb-3">📊 Where Do You Stand?</div>
        <div class="mb-4">
          <label class="block text-xs text-gray-400 mb-1">Monthly Downloads</label>
          <input type="range" id="benchmark-downloads" min="0" max="100000" value="1000" 
                 class="w-full h-2 bg-dark-700 rounded-lg appearance-none cursor-pointer"
                 oninput="updateBenchmark()">
          <div class="flex justify-between text-xs text-gray-500 mt-1">
            <span>0</span>
            <span id="benchmark-downloads-value">1,000</span>
            <span>100K</span>
          </div>
        </div>

        <div class="mb-4">
          <label class="block text-xs text-gray-400 mb-1">Average Price (Minecoins)</label>
          <select id="benchmark-price" class="w-full px-3 py-2 bg-dark-700 rounded text-sm" onchange="updateBenchmark()">
            <option value="490">490 ($2.99) — Skin Pack</option>
            <option value="830" selected>830 ($4.99) — Small Map</option>
            <option value="1340">1340 ($7.99) — Large Map/Mod</option>
            <option value="1690">1690 ($9.99) — Premium Content</option>
          </select>
        </div>

        <div id="benchmark-results" class="bg-dark-700 rounded-lg p-3">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm">Your Estimated Tier:</span>
            <span id="benchmark-tier" class="px-2 py-1 bg-accent-blue rounded text-xs font-semibold">Established Creator</span>
          </div>
          <div class="text-xs text-gray-400 mb-2">Monthly Revenue Estimate:</div>
          <div id="benchmark-revenue" class="text-2xl font-bold text-accent-green mb-2">$2,490</div>
          <div class="text-xs text-gray-500">After platform fees (~30%)</div>
        </div>
      </div>

      <div class="bg-dark-800 rounded-lg p-3 mb-4">
        <div class="text-sm font-semibold mb-2">🏆 Creator Tiers</div>
        <div class="space-y-2">
          ${creatorTiers.map(tier => `
            <div class="flex items-center justify-between text-xs">
              <div class="flex items-center gap-2">
                <div class="w-3 h-3 rounded-full bg-${tier.color}-500"></div>
                <span>${tier.name}</span>
              </div>
              <span class="text-gray-400">${tier.minSales.toLocaleString()} - ${tier.maxSales.toLocaleString()} sales</span>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <div class="flex gap-2">
        <a href="https://www.minecraft.net/en-us/marketplace" target="_blank" rel="noopener" class="px-3 py-2 bg-accent-blue hover:bg-blue-600 rounded text-sm transition-colors">🏪 Marketplace</a>
        <a href="https://oasisaiminecraft.com/statistics/minecraft-economic-marketplace-analytics/" target="_blank" rel="noopener" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">📊 Analytics Source</a>
      </div>
    </div>
  `;
...
function updateBenchmark() {
  const downloads = parseInt(document.getElementById('benchmark-downloads')?.value || 1000);
  const price = parseInt(document.getElementById('benchmark-price')?.value || 830);
  
  // Update display
  const downloadsValue = document.getElementById('benchmark-downloads-value');
  if (downloadsValue) downloadsValue.textContent = downloads.toLocaleString();
  
  // Calculate revenue (platform takes ~30%)
  const grossRevenue = downloads * price * 0.006; // Minecoin to USD conversion approx
  const netRevenue = grossRevenue * 0.7;
  
  // Determine tier
  let tier = creatorTiers[0];
  for (const t of creatorTiers) {
    if (downloads >= t.minSales && downloads <= t.maxSales) {
      tier = t;
      break;
    }
  }
  ...
  // Update results
  const tierEl = document.getElementById('benchmark-tier');
  const revenueEl = document.getElementById('benchmark-revenue');
  
  if (tierEl) {
    tierEl.textContent = tier.name;
    tierEl.className = `px-2 py-1 bg-accent-${tier.color} rounded text-xs font-semibold`;
  }
...
document.getElementById('tab-btn-youtube')?.addEventListener('click', () => {
  ...
  setTimeout(initMCMovieContentIdeas, 700);
});

// ==================== MINECRAFT MOVIE CONTENT IDEAS ====================

const mcMovieContentIdeas = [
  {
    title: 'I Watched the Minecraft Movie 10 Times',
    type: 'Reaction/Challenge',
    hook: 'What happens when you watch the billion-dollar movie on repeat?',
    format: 'Vlog + Commentary',
    estimatedViews: '2-10M'
  },
  {
    title: 'Recreating Every Scene from the Minecraft Movie',
    type: 'Build/Recreate',
    hook: 'Building the entire movie in Minecraft block by block',
    format: 'Timelapse + Cinematic',
    estimatedViews: '5-20M'
  },
  {
    title: 'Minecraft Movie Easter Eggs You Missed',
    type: 'Analysis',
    hook: 'Hidden references and secrets only true fans caught',
    format: 'Breakdown + Screenshots',
    estimatedViews: '3-15M'
  },
  {
    title: 'The Minecraft Movie Made $1 Billion. Here\'s Why.',
    type: 'Analysis/Commentary',
    hook: 'Breaking down the marketing and cultural phenomenon',
    format: 'Documentary Style',
    estimatedViews: '1-5M'
  },
  {
    title: 'I Built the Minecraft Movie in BBS Mod',
    type: 'Cinematic/Build',
    hook: 'Full recreation with Blockbuster Studio mod',
    format: 'Showcase + Tutorial',
    estimatedViews: '2-8M'
  },
  {
    title: 'Reacting to Minecraft Movie Trailer vs Reality',
    type: 'Comparison',
    hook: 'Did the final movie live up to the hype?',
    format: 'Side-by-side',
    estimatedViews: '1-4M'
  },
  {
    title: 'Minecraft Movie Speedrun — Watch to Credits',
    type: 'Challenge',
    hook: 'How fast can I watch the entire movie?',
    format: 'Speedrun Format',
    estimatedViews: '500K-2M'
  },
  {
    title: 'Making the Minecraft Movie\'s Best Scene in Minecraft',
    type: 'Build/Tutorial',
    hook: 'Step-by-step recreation of iconic moments',
    format: 'Tutorial',
    estimatedViews: '3-10M'
  }
];

const mcMovieStats = {
  domesticBoxOffice: 423.9, // million
  globalBoxOffice: 816, // million (approaching $1B)
  openingWeekend: 157, // million
  daysTo200M: 7,
  rtScore: 48, // Rotten Tomatoes
  pvodRank: 1
};

function initMCMovieContentIdeas() {
  const container = document.getElementById('mc-movie-content-ideas');
  if (!container) return;
    ...
    <div class="card rounded-lg p-4 border-l-4 border-accent-green">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          <div class="text-3xl">🎬</div>
          <div>
            <h3 class="font-semibold text-lg">Minecraft Movie Content Ideas</h3>
            <p class="text-sm text-gray-400">$816M+ global box office — massive content opportunity</p>
          </div>
        </div>
        <button onclick="generateMCMovieIdea()" class="mt-2 md:mt-0 px-3 py-2 bg-accent-green hover:bg-green-600 rounded text-sm transition-colors">🎲 Generate Idea</button>
      </div>

      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-green">$816M</div>
          <div class="text-xs text-gray-400">Global Box Office</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-blue">$423M</div>
          <div class="text-xs text-gray-400">Domestic (2025 #1)</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-yellow">7 Days</div>
          <div class="text-xs text-gray-400">To $200M</div>
        </div>
        <div class="bg-dark-800 rounded-lg p-3 text-center">
          <div class="text-xl font-bold text-accent-purple">#1</div>
          <div class="text-xs text-gray-400">On PVOD</div>
        </div>
      </div>

      <div class="bg-dark-800 rounded-lg p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-semibold">📈 Performance Context</span>
          <span class="text-xs px-2 py-1 bg-accent-green/20 text-accent-green rounded">Trending</span>
        </div>
        <p class="text-xs text-gray-300">
          Despite 48% Rotten Tomatoes score, the Minecraft Movie became Warner Bros.' highest-grossing 2025 film. 
          It dominated the box office for 16 of 39 weekends and is now trending #1 on PVOD. 
          Cultural impact > critical reception.
        </p>
      </div>

      <div id="mc-movie-idea-output" class="hidden bg-dark-800 rounded-lg p-4 mb-4">
        <div class="flex items-center justify-between mb-3">
          <h4 class="font-semibold">🎯 Content Idea</h4>
          <button onclick="copyMCMovieIdea()" class="text-xs px-2 py-1 bg-accent-blue rounded hover:bg-blue-600">📋 Copy</button>
        </div>
        <div id="mc-movie-idea-content"></div>
      </div>

      <div class="bg-dark-800 rounded-lg p-3">
        <div class="text-sm font-semibold mb-2">💡 All Content Ideas</div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          ${mcMovieContentIdeas.map((idea, i) => `
            <div class="bg-dark-700 rounded p-2 text-xs cursor-pointer hover:bg-dark-600" onclick="selectMCMovieIdea(${i})">
              <div class="flex justify-between items-start">
                <span class="font-semibold">${idea.title}</span>
                <span class="text-accent-green">${idea.estimatedViews}</span>
              </div>
              <div class="text-gray-400 mt-1">${idea.type} • ${idea.format}</div>
            </div>
          `).join('')}
        </div>
      </div>
      ...
      <div class="mt-4 flex gap-2">
        <a href="https://en.wikipedia.org/wiki/A_Minecraft_Movie" target="_blank" rel="noopener" class="px-3 py-2 bg-accent-blue hover:bg-blue-600 rounded text-sm transition-colors">📖 Wikipedia</a>
        <a href="https://www.goldderby.com/film/2025/2025-box-office-winners-losers-minecraft-wicked-snow-white/" target="_blank" rel="noopener" class="px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded text-sm transition-colors">📊 Box Office Data</a>
      </div>
    </div>
  `;
}

let selectedMCMovieIdea = null;

function selectMCMovieIdea(index) {
  selectedMCMovieIdea = mcMovieContentIdeas[index];
  displayMCMovieIdea();
}

function generateMCMovieIdea() {
  selectedMCMovieIdea = mcMovieContentIdeas[Math.floor(Math.random() * mcMovieContentIdeas.length)];
  displayMCMovieIdea();
}

function displayMCMovieIdea() {
  if (!selectedMCMovieIdea) return;
  
  const output = document.getElementById('mc-movie-idea-output');
  const content = document.getElementById('mc-movie-idea-content');
  
  content.innerHTML = `
    <div class="mb-3">
      <div class="text-xs text-gray-400 mb-1">Title</div>
      <div class="font-semibold text-accent-green">${selectedMCMovieIdea.title}</div>
    </div>
    
    <div class="mb-3">
      <div class="text-xs text-gray-400 mb-1">Hook</div>
      <div class="text-sm">${selectedMCMovieIdea.hook}</div>
    </div>
    
    <div class="grid grid-cols-2 gap-3">
      <div class="bg-dark-700 rounded p-2">
        <div class="text-xs text-gray-400">Type</div>
        <div class="text-sm font-semibold">${selectedMCMovieIdea.type}</div>
      </div>
      <div class="bg-dark-700 rounded p-2">
        <div class="text-xs text-gray-400">Format</div>
        <div class="text-sm font-semibold">${selectedMCMovieIdea.format}</div>
      </div>
    </div>
    
    <div class="mt-3 bg-dark-700 rounded p-2">
      <div class="text-xs text-gray-400">Estimated Views</div>
      <div class="text-lg font-bold text-accent-yellow">${selectedMCMovieIdea.estimatedViews}</div>
    </div>
  `;
...
function copyMCMovieIdea() {
  if (!selectedMCMovieIdea) return;
  
  const text = `MINECRAFT MOVIE CONTENT IDEA

Title: ${selectedMCMovieIdea.title}
Type: ${selectedMCMovieIdea.type}
Format: ${selectedMCMovieIdea.format}

Hook: ${selectedMCMovieIdea.hook}

Estimated Views: ${selectedMCMovieIdea.estimatedViews}

Context:
• Minecraft Movie: $816M+ global box office
• Domestic: $423.9M (2025 #1)
• Trending #1 on PVOD
...
`;
  ...
  navigator.clipboard.writeText(text).then(() => {
    alert('Content idea copied to clipboard!');
  });
}
...
const engagementSignals = [
  { name: 'Likes', weight: 15, description: 'Quick positive signal', multiplier: 1.0 },
  { name: 'Comments', weight: 25, description: 'High engagement indicator', multiplier: 2.5 },
  { name: 'Shares', weight: 20, description: 'Viral potential signal', multiplier: 3.0 },
  { name: 'Saves', weight: 20, description: 'Long-term value indicator', multiplier: 2.0 },
  { name: 'CTR', weight: 20, description: 'Thumbnail/title effectiveness', multiplier: 1.5 }
];

const engagementPresets = [
  { name: 'Viral', likes: 50000, comments: 5000, shares: 10000, saves: 8000, ctr: 12 },
  { name: 'Strong', likes: 10000, comments: 1000, shares: 2000, saves: 1500, ctr: 8 },
  { name: 'Average', likes: 1000, comments: 100, shares: 200, saves: 150, ctr: 5 },
  { name: 'Poor', likes: 100, comments: 10, shares: 20, saves: 15, ctr: 2 }
];

function initEngagementOptimizer() {
  const container = document.getElementById('engagement-optimizer');
  if (!container) return;
  
  container.innerHTML = `
    <div class="card rounded-lg p-4 border-l-4 border-accent-blue">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div class="flex items-center gap-3">
          <div class="text-3xl">📈</div>
          <div>
            <h3 class="font-semibold text-lg">YouTube Engagement Optimizer</h3>
            <p class="text-sm text-gray-400">2026 Algorithm — Community Engagement Signals</p>
          </div>
        </div>
        <button onclick="calculateEngagementScore()" class="mt-2 md:mt-0 px-3 py-2 bg-accent-blue hover:bg-blue-600 rounded text-sm transition-colors">🎯 Calculate Score</button>
      </div>

      <div class="bg-dark-800 rounded-lg p-3 mb-4">
        <div class="text-sm font-semibold mb-2">⚡ 2026 Algorithm Priority: Community Engagement</div>
        <p class="text-xs text-gray-300">
          YouTube's 2026 algorithm prioritizes <strong>viewer satisfaction signals</strong> over raw watch time. 
          Likes, comments, shares, and saves are now weighted heavily in recommendations. 
          Authentic engagement often beats subscriber count.
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ${engagementSignals.map(sig => `
          <div class="bg-dark-800 rounded-lg p-3">
            <div class="text-xs text-gray-400">${sig.name}</div>
            <div class="text-lg font-bold text-accent-blue">${sig.weight}%</div>
            <div class="text-xs text-gray-500">${sig.description}</div>
          </div>
        `).join('')}
      </div>

      <div class="bg-dark-800 rounded-lg p-4 mb-4">
        <div class="text-sm font-semibold mb-3">📊 Your Engagement Metrics</div>
        
        <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1">Likes</label>
            <input type="number" id="eng-likes" value="1000" class="w-full px-2 py-1 bg-dark-700 rounded text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">Comments</label>
            <input type="number" id="eng-comments" value="100" class="w-full px-2 py-1 bg-dark-700 rounded text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">Shares</label>
            <input type="number" id="eng-shares" value="50" class="w-full px-2 py-1 bg-dark-700 rounded text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">Saves</label>
            <input type="number" id="eng-saves" value="75" class="w-full px-2 py-1 bg-dark-700 rounded text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1">CTR %</label>
            <input type="number" id="eng-ctr" value="5" class="w-full px-2 py-1 bg-dark-700 rounded text-sm">
          </div>
        </div>

        <div class="mb-3">
          <span class="text-xs text-gray-400">Quick Presets:</span>
          <div class="flex flex-wrap gap-2 mt-1">
            ${engagementPresets.map(p => `
              <button onclick="applyEngagementPreset('${p.name}')" class="px-2 py-1 bg-dark-700 hover:bg-dark-600 rounded text-xs transition-colors">${p.name}</button>
            `).join('')}
          </div>
        </div>
      </div>

      <div id="engagement-result" class="hidden bg-dark-800 rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          <h4 class="font-semibold">🎯 Engagement Score</h4>
          <button onclick="copyEngagementAnalysis()" class="text-xs px-2 py-1 bg-accent-blue rounded hover:bg-blue-600">📋 Copy</button>
        </div>
        <div id="engagement-score-display" class="text-center mb-4"></div>
        <div id="engagement-breakdown"></div>
      </div>

      <div class="mt-4 bg-dark-800 rounded-lg p-3">
        <div class="text-sm font-semibold mb-2">💡 2026 Engagement Tips</div>
        <ul class="text-xs text-gray-300 space-y-1">
          <li>• Ask questions in videos to drive comments (highest weight signal)</li>
          <li>• Create "save-worthy" content (tutorials, reference material)</li>
          <li>• Include shareable moments (surprising facts, emotional beats)</li>
          <li>• End screens should prompt specific actions (not just "subscribe")</li>
          <li>• Reply to early comments to boost discussion</li>
        </ul>
      </div>
    </div>
  `;
...
function applyEngagementPreset(presetName) {
  const preset = engagementPresets.find(p => p.name === presetName);
  if (!preset) return;
  
  document.getElementById('eng-likes').value = preset.likes;
  document.getElementById('eng-comments').value = preset.comments;
  document.getElementById('eng-shares').value = preset.shares;
  document.getElementById('eng-saves').value = preset.saves;
  document.getElementById('eng-ctr').value = preset.ctr;
  
  calculateEngagementScore();
}

function calculateEngagementScore() {
  const likes = parseInt(document.getElementById('eng-likes')?.value || 0);
  const comments = parseInt(document.getElementById('eng-comments')?.value || 0);
  const shares = parseInt(document.getElementById('eng-shares')?.value || 0);
  const saves = parseInt(document.getElementById('eng-saves')?.value || 0);
  const ctr = parseFloat(document.getElementById('eng-ctr')?.value || 0);
  
  // Normalize metrics (scale to 0-100)
  const likesScore = Math.min(likes / 1000, 100) * 0.15;
  const commentsScore = Math.min(comments / 200, 100) * 0.25;
  const sharesScore = Math.min(shares / 100, 100) * 0.20;
  const savesScore = Math.min(saves / 100, 100) * 0.20;
  const ctrScore = Math.min(ctr / 10, 100) * 0.20;
  
  const totalScore = likesScore + commentsScore + sharesScore + savesScore + ctrScore;
  
  let rating = 'Poor';
  let color = 'red';
  if (totalScore >= 80) { rating = 'Viral'; color = 'purple'; }
  else if (totalScore >= 60) { rating = 'Strong'; color = 'green'; }
  else if (totalScore >= 40) { rating = 'Average'; color = 'yellow'; }
  
  const resultDiv = document.getElementById('engagement-result');
  const scoreDisplay = document.getElementById('engagement-score-display');
  const breakdownDiv = document.getElementById('engagement-breakdown');
  
  scoreDisplay.innerHTML = `
    <div class="text-5xl font-bold text-accent-${color} mb-2">${Math.round(totalScore)}/100</div>
  ...
  breakdownDiv.innerHTML = `
    <div class="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
      <div class="bg-dark-700 rounded p-2">
        ...
        <div class="font-semibold">${Math.round(likesScore)}/15</div>
      </div>
      <div class="bg-dark-700 rounded p-2">
        <div class="text-gray-400">Comments</div>
        <div class="font-semibold">${Math.round(commentsScore)}/25</div>
      </div>
      <div class="bg-dark-700 rounded p-2">
        <div class="text-gray-400">Shares</div>
        <div class="font-semibold">${Math.round(sharesScore)}/20</div>
      </div>
      <div class="bg-dark-700 rounded p-2">
        <div class="text-gray-400">Saves</div>
        <div class="font-semibold">${Math.round(savesScore)}/20</div>
      </div>
      <div class="bg-dark-700 rounded p-2">
        <div class="text-gray-400">CTR</div>
        <div class="font-semibold">${Math.round(ctrScore)}/20</div>
      </div>
    </div>
  `;
  ...
  window.currentEngagementScore = {
    total: Math.round(totalScore),
    rating,
    likes: Math.round(likesScore),
    comments: Math.round(commentsScore),
    shares: Math.round(sharesScore),
    saves: Math.round(savesScore),
    ctr: Math.round(ctrScore)
  };
}

function copyEngagementAnalysis() {
  if (!window.currentEngagementScore) return;
  
  const s = window.currentEngagementScore;
  const text = `YOUTUBE ENGAGEMENT ANALYSIS

Overall Score: ${s.total}/100 (${s.rating})

Breakdown:
• Likes: ${s.likes}/15 (15% weight)
• Comments: ${s.comments}/25 (25% weight)
• Shares: ${s.shares}/20 (20% weight)
• Saves: ${s.saves}/20 (20% weight)
• CTR: ${s.ctr}/20 (20% weight)

2026 Algorithm Notes:
• Community engagement > subscriber count
• Comments are highest-weight signal
• Saves indicate long-term value
• Authentic engagement beats artificial inflation

Generated by Nox Dashboard — ${new Date().toLocaleDateString()}
`;
  
  navigator.clipboard.writeText(text).then(() => {
    alert('Engagement analysis copied to clipboard!');
  });
}

// Auto-init when YouTube tab is clicked
document.getElementById('tab-btn-youtube')?.addEventListener('click', () => {
  setTimeout(initEngagementOptimizer, 800);
});
...
window.initNVDACountdown = initNVDACountdown;
window.copyNVDANotes = copyNVDANotes;
window.initMC26Planner = initMC26Planner;
window.copyMC26Ideas = copyMC26Ideas;
window.generateMC26Script = generateMC26Script;
window.initShortsLongMixer = initShortsLongMixer;
window.selectMixTemplate = selectMixTemplate;
window.generateContentMix = generateContentMix;
window.copyMixPlan = copyMixPlan;
window.initBBSCinematicPlanner = initBBSCinematicPlanner;
window.selectScene = selectScene;
window.selectCamera = selectCamera;
window.generateSceneSequence = generateSceneSequence;
...
window.initMCMovieContentIdeas = initMCMovieContentIdeas;
window.selectMCMovieIdea = selectMCMovieIdea;
window.generateMCMovieIdea = generateMCMovieIdea;
window.copyMCMovieIdea = copyMCMovieIdea;
window.initEngagementOptimizer = initEngagementOptimizer;
window.applyEngagementPreset = applyEngagementPreset;
window.calculateEngagementScore = calculateEngagementScore;
window.copyEngagementAnalysis = copyEngagementAnalysis;

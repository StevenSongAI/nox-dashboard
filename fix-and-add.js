...
try {
  ...
  // The issue is: researchTools ends with }, then a bare { starts the bonusBrief
  // We need to either remove the bonusBrief or put it in contentBriefs
  
  // Find the researchTools section and fix it
  const researchToolsEnd = rawData.lastIndexOf('"cronSchedule": "7AM and 6PM daily"');
  if (researchToolsEnd === -1) {
    throw new Error('Cannot find researchTools end');
  }
  
  // Find the position after the closing brace and comma of researchTools
  const afterResearchTools = rawData.indexOf('}', researchToolsEnd) + 1;
  const beforeBonusBrief = rawData.indexOf('{', afterResearchTools);
  
  // Extract the bonusBrief object
  let braceCount = 0;
  let inString = false;
  let escapeNext = false;
  let bonusBriefStart = beforeBonusBrief;
  let bonusBriefEnd = bonusBriefStart;
  
  for (let i = bonusBriefStart; i < rawData.length; i++) {
  ...
  // Extract the bonusBrief
  const bonusBriefJson = rawData.substring(bonusBriefStart, bonusBriefEnd);
  const bonusBrief = JSON.parse(bonusBriefJson);
  
  console.log('Extracted bonusBrief:', bonusBrief.id);
  
  // Now reconstruct the JSON properly
  const before = rawData.substring(0, afterResearchTools);
  const after = rawData.substring(bonusBriefEnd);
  
  // Parse what we have so far
  const tempData = JSON.parse(before + '}');
  
  // Add bonusBrief to contentBriefs
  if (!tempData.contentBriefs) {
    tempData.contentBriefs = [];
  }
  tempData.contentBriefs.push(bonusBrief);
  
  // Parse the rest (synthesizedInsights)
  const restData = JSON.parse(after.trim().startsWith(',') ? '{' + after.substring(1) : after);
  
  // Merge
  Object.assign(tempData, restData);
